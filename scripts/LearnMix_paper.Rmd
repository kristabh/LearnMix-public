...
# 
packages <- c("dplyr", "readr", "ggplot2", "forcats", "stringr", "lme4", "corrr", "ez", "apa", "english", "lmerTest", "devtools", "rstatix")
# 
...
library(devtools)
library(rstatix)
library(lazyeval) #need for interp function
library(devtools)
library(here)
...
mydata2 <- dataset2 %>%
  ...
  left_join(edu.categories) %>%
  mutate(object.order = NA) # Set object.order and target.image to NA for now
...
response_window_agg_by_sub_test <- make_time_window_data(response_window_clean_test, 
                                                   ...
                                                   # predictor_columns = my_predictors,
                                                    summarize_by = "id",
...
#Two princeton babies were miscoded as "keepers" but don't have data

part_princeton[part_princeton$id == "NJ24", "keeper"] <- 0
part_princeton[part_princeton$id == "NJ25", "keeper"] <- 0

# Filter out these babies, add educational categories
part_princeton <- part_princeton %>%
  filter(keeper == 1) %>%
   left_join(edu.categories)
...
all_clean_training <- full_join(response_window_clean_training, response_window_clean_training2) %>%
  left_join(edu.categories)

# Some hoops due to eyetracking R
all_clean_training_colnames <- colnames(all_clean_training)
mostattributes(all_clean_training) <- attributes(response_window_clean_training)
colnames(all_clean_training) <- all_clean_training_colnames



#esponse_window_agg_by_trial_training_all <- full_join(response_window_agg_by_trial_training, response_window_agg_by_trial_training2)
...
# response_window_agg_by_trial_training_all <- make_time_window_data(all_clean_training, 
#                                                              aois = c('target'), 
#                                                              predictor_columns=c("sentence.type", "carrier.dominant", "population", "vocab.dom", "vocab.nondom", "lang.mix", "edu.years"),
#                                                          #    summarize_by = c("id"),
#                                                              other_dv_columns = c("PupilLeft", "PupilRight")) %>%  
#   mutate(AOI = as.factor(AOI)) %>%
#   mutate(familiarity = as.factor(recode(AOI, distractorfamiliar = "familiar", targetfamiliar = "familiar", distractornovel = "novel", targetnovel = "novel"))) %>%
...
#   full_join(response_window_clean_test2) %>%
#   left_join(edu.categories)
# 
# # Some hoops due to eyetracking R
# all_clean_test_colnames <- colnames(all_clean_test)
# mostattributes(all_clean_test) <- attributes(response_window_clean_test)
# colnames(all_clean_test) <- all_clean_test_colnames
# 
# 
# 
# response_window_agg_by_trial_test_all <- make_time_window_data(all_clean_test, 
#                                                              aois = c('target'), 
#                                                              predictor_columns=c("sentence.type", "carrier.dominant", "population", "vocab.dom", "vocab.nondom", "lang.mix", "edu.years"),
#                                                          #    summarize_by = c("id"),
#                                                              other_dv_columns = c("PupilLeft", "PupilRight")) %>%  
...
# 
# response_window_agg_by_sub_training <- make_time_window_data(all_clean_training, 
#                                                              aois = c('target'), 
#                                                              predictor_columns=c("sentence.type", "carrier.dominant", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
#                                                              summarize_by = c("id"),
#                                                              other_dv_columns = c("PupilLeft", "PupilRight")) %>%  
...
# response_window_agg_by_sub_test <- make_time_window_data(all_clean_test, 
#                                                              aois = c('target'), 
#                                                              predictor_columns=c("sentence.type", "carrier.dominant", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
#                                                              summarize_by = c("id"),
#                                                              other_dv_columns = c("PupilLeft", "PupilRight")) %>%  
