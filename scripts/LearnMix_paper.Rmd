...
list.files(here("eyetrackingR"), full.names = TRUE) %>% walk(source)
...
```{r test-figure, include = TRUE, fig.cap = "Average proportion looking to target object for French-English bilinguals (Experiment 1). Circles show individual data points. Bars represent standard error of the mean."}
...
```{r training-timecourse-figure, echo = FALSE, include = TRUE, fig.cap = "Timecourse of looking to the labeled target for French-English bilingual children (Experiment 1)."}
...
```{r test-figure2, include = TRUE, fig.cap = "Average proportion looking to target object for Spanish-English bilinguals (Experiment 2). Circles show individual data points. Bars represent standard error of the mean."}
...
Next, we directly compared data for French-English (Experiment 1) and Spanish-English (Experiment 2) bilingual children. First, we examined performance during the test phase in a 2 (population: French-English, Spanish-English) x 2 (sentence type: single-language, mixed-language) mixed ANOVA (Table \@ref(tab:test-comparison-anova-table)). The largest observed effect was an interaction of population and sentence type, which was expected as French-English bilinguals looked longer at the novel target that had been encountered in single-language trials than in mixed-language trials, but Spanish-English bilinguals showed similar looking to the targets regardless of of the presence or absence of mixing. However, this interaction did not reach statistical significance (*p* = `r int_p `).
...
```{r correl-figure, include = TRUE, fig.cap = "Scatterplot showing performance during the learning phase and performance at test by population and sentence type. Lines represent best linear fit."}
comb_correl_plot
```
...
#Run correlation with everybody all together, then merge in the correlations by group
all_corr_test <- response_window_agg_by_sub_test_all %>%
  select(-population) %>%
...
#Test correlations separately by phase

grouped_corr_test_sentence <- response_window_agg_by_sub_test_all %>%
  group_by(population, sentence.type) %>%
  rstatix::cor_test(Prop, vocab.dom, vocab.nondom, vocab.total, lang.mix, edu.years) %>%
  ungroup() %>% filter(var1 == "Prop") %>%
  filter(var2 != "Prop")


all_corr_test_sentence <- response_window_agg_by_sub_test_all %>%
  select(-population) %>%
  mutate(population = "All") %>%
  group_by(population) %>%
  rstatix::cor_test(Prop, vocab.dom, vocab.nondom, vocab.total, lang.mix, edu.years) %>%
...
# Have to make new df for Montreal population only - only target and distractor (not novel/familiar). Other dfs are okay to use.
response_window_agg_by_sub_training_t <- make_time_window_data(response_window_clean_training, 
                                                    aois = c('target'), 
                                                    predictor_columns=c("sentence.type", "carrier.dominant", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
                                                    summarize_by = "id",
                                                    other_dv_columns = c("PupilLeft", "PupilRight")) %>%  
...
response_window_agg_by_sub_training_all <- full_join(response_window_agg_by_sub_training_t, response_window_agg_by_sub_training2)
...
all_corr_training <- response_window_agg_by_sub_training_all %>%
  select(-population) %>%
  mutate(population = "All") %>%
  group_by(population) %>%
  rstatix::cor_test(Prop, vocab.dom, vocab.nondom, vocab.total, lang.mix, edu.years) %>%
...
all_corr <- bind_rows(all_corr_training, all_corr_test) %>%
  ...
  mutate(variable = fct_relevel(variable, c("lang.mix","vocab.dom", "vocab.nondom", "vocab.total", "edu.years"))) %>%
...
##Computing ICCs for test

#Montreal data

response_window_agg_by_trial_test <- make_time_window_data(response_window_clean_test, 
                                                    aois = c("target"),
                                                    predictor_columns=c("sentence.type", "carrier.dominant", "object.order", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
                                                  #  summarize_by = "id",
                                                    other_dv_columns = c("PupilLeft", "PupilRight", "teelo", "walem"))
...
response_window_agg_by_trial_test2 <- make_time_window_data(response_window_clean_test2, 
                                                    aois = c("target"),
                                                    predictor_columns=c("sentence.type", "carrier.dominant", "object.order", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
                                                  #  summarize_by = "id",
                                                    other_dv_columns = c("teelo", "walem"))


#ICC for all test trials

all_trial_test <- full_join(response_window_agg_by_trial_test, response_window_agg_by_trial_test2) %>%
  select(id, sentence.type, Prop, trial.number) %>%
  mutate(Prop = if_else(is.nan(Prop), NA_real_, Prop))


icc_all_test <- all_trial_test %>%
  pivot_wider(id_cols = id, names_from  = trial.number, values_from = Prop) %>%
  select(-id)

ICC(icc_all_test, missing = FALSE, lmer = TRUE) # ICC3k = .81 - this is good

# Extract ICC values for test

test_icc <- ICC(icc_all_test, missing = FALSE, lmer = TRUE) # ICC2k = .79 - this is good

test_icc_estimate <- test_icc$results$ICC[6]
test_icc_lower <- test_icc$results$`lower bound`[6]
test_icc_upper <- test_icc$results$`upper bound`[6]

#Some additional checks to look at each trial type separately

icc_single_test <- all_trial_test %>%
  filter(sentence.type == "Single") %>%
  group_by(id) %>%
  mutate(trial.count = row_number()) %>%
  ungroup() %>%
  pivot_wider(id_cols = id, names_from  = trial.count, values_from = Prop) %>%
  select(-id)

ICC(icc_single_test, missing = FALSE, lmer = TRUE) # ICC3k = .58 - this is okay


icc_mixed_test <- all_trial_test %>%
  filter(sentence.type == "Mixed") %>%
  group_by(id) %>%
  mutate(trial.count = row_number()) %>%
  ungroup() %>%
...
ICC(icc_mixed_test, missing = FALSE, lmer = TRUE) # ICC3k = .86 - this is good
...
```{r}
##Computing ICCs for training


# Montreal data
response_window_agg_by_trial_training <- make_time_window_data(response_window_clean_training, 
                                                    aois = c("target"),
                                                    predictor_columns=c("sentence.type", "carrier.dominant", "object.order", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
                                                  #  summarize_by = "id",
                                                    other_dv_columns = c("PupilLeft", "PupilRight", "teelo", "walem"))
...
#NJ data
response_window_agg_by_trial_training2 <- make_time_window_data(response_window_clean_training2, 
                                                    aois = c("target"),
                                                    predictor_columns=c("sentence.type", "carrier.dominant", "object.order", "population", "vocab.dom", "vocab.nondom", "vocab.total", "lang.mix", "edu.years"),
                                                  #  summarize_by = "id",
                                                    other_dv_columns = c("teelo", "walem"))
...
all_trial_training <- full_join(response_window_agg_by_trial_training, response_window_agg_by_trial_training2)

icc_all_training <- all_trial_training %>%
  select(id, sentence.type, Prop, trial.number) %>%
  mutate(Prop = if_else(is.nan(Prop), NA_real_, Prop)
  ) %>%
  pivot_wider(id_cols = id, names_from  = trial.number, values_from = Prop) %>%
...
train_icc <- ICC(icc_all_training, missing = FALSE, lmer = TRUE) # ICC2k = .79 - this is good

train_icc_estimate <- train_icc$results$ICC[6]
train_icc_lower <- train_icc$results$`lower bound`[6]
train_icc_upper <- train_icc$results$`upper bound`[6]


icc_single_training <- all_trial_training %>%
  filter(sentence.type == "Single") %>%
  group_by(id) %>%
  mutate(trial.count = row_number()) %>%
  ungroup() %>%
...
ICC(icc_single_training, missing = FALSE, lmer = TRUE) # ICC3k = .72 - this is good


icc_mixed_training <- all_trial_training %>%
  filter(sentence.type == "Mixed") %>%
  group_by(id) %>%
  mutate(trial.count = row_number()) %>%
  ungroup() %>%
...
ICC(icc_mixed_training, missing = FALSE, lmer = TRUE) # ICC2k = .67 - this is okay
...
# For just one group
pwr_singlegroup <- round(pwr.r.test(n = 19, power = .8)$r,2)
pwr_combined <- round(pwr.r.test(n = 38, power = .8)$r,2)
...
Next, we were interested in whether performance was related to vocabulary knowledge (dominant language vocabulary, non-dominant language vocabulary, and total vocabulary), exposure to language mixing, or socioeconomic status. As a first step, we computed reliability estimates for infants’ performance in the learning and test phases. The reliability of proportion target looking across the 16 learning trials was estimated with an intraclass correlation coefficient (ICC), based on a mean-rating (k = 16), consistency, 2-way random-effects model (ICC3k) using the psych package in R [@psych_package]. The estimated ICC for training trials was `r train_icc_estimate`, 95% CI [`r train_icc_lower`, `r train_icc_upper`]. Using a similar approach for proportion target looking during the 4 test trials (k = 4), the estimated ICC for test trials was `r test_icc_estimate`, 95% CI [`r test_icc_lower`, `r test_icc_upper`]. These values reflect good reliability [@koo_2016], indicating that children showed stable individual variability in our task, which could potentially be accounted for by our predictors of interest [for a discussion of the role of reliability in infant research, see @byers-heinlein_2021].

Next, we performed a sensitivity power analysis to determine the magnitude of correlations we would be able to detect given our sample size with 80% power, using the pwr package in R [@pwr_package]. The analysis indicated that we could detect an observed correlation of *r* = `r pwr_singlegroup` for participants within each community (*n* = 19/community) and *r* = `r pwr_combined` for the two groups combined (*N* = 38). Thus, we had sufficient power to detect medium to large correlations, with greater power for the combined sample than for each community separately.

Finally, we computed correlations with the predictors of interest separately for the learning and test phases, both for each community separately as well as for the two communities combined. Thus, a total of `r n_corr` different correlations were computed (see Table \@ref(tab:corr-table)). Two of the correlations approached but did not reach statistical significance, although not when corrected for multiple comparisons. Specifically, there were positive correlations in the combined sample between performance on the learning phase and total vocabulary size (*p* = `r learning_vocab`), as well as performance at test and maternal education (*p* = `r test_matedu`). None of the other correlations approached statistical significance, even without controlling for multiple comparisons (*p*s ⩾ .10).
...
Our investigation was motivated by the idea that language mixing can sometimes derail children’s recognition of a familiar word, which in turn could derail learning of a novel word that occurs moments later. Indeed, substantial previous work has demonstrated that both children and adults experience processing costs for language mixing in a range of paradigms including eye tracking and speech production [@altarriba_1996; @byersh_2017; @chan_1983; @grainger_1988; @macnamara_1971; @meuter_1999; @moreno_2002; @potter_2019; @price_1999; @proverbio_2004; @soares_1984; @thomas_2000; @morini_2019], and our stimuli were closely modeled after those used in previous studies. However, our analyses of children’s looking behavior during the learning phase did not necessarily converge with these findings. We found that children in both groups did successfully identify the familiar object during the learning phase, regardless of whether they encountered its label in a single-language or a mixed-language sentence. Eye-tracking data, which was only available for French-English bilinguals, additionally revealed that French-English bilinguals successfully directed their gaze to the target novel object itself. That is, children across both communities generally shifted their eyes to the ‘correct’ location independent of sentence type, community, or their previous experience with language mixing. This presents a key problem: despite consistently successful looking toward the correct familiar/novel referent pair, Spanish-English bilinguals did not appear to successfully learn either of the novel word-object mappings (perhaps because they were less efficient overall in orienting to the target novel referent), yet French-English bilinguals showed asymmetry in doing so across the two sentence types.
...
A second possibility is that bilingual children learn over time that language mixing does not offer an optimal moment for learning, and thus they either implicitly or explicitly disengage and/or do not encode novel words that appear in mixed-language sentences. Adult bilinguals sometimes mix their languages when they have a lexical gap [@grosjean_1982] or have difficulty retrieving a word [@heredia_2001]. Given evidence that toddlers prefer learning word labels that come from certain over uncertain speakers [@birch_2008, although see also @buac_2019], it may be that children interpret some types of language mixing as a signal of a speaker’s lexical uncertainty in the moment, which may hinder real-time encoding of a novel label. This possibility would need to be tested empirically in a future study that broadly examines children’s uptake of new information in communicative contexts with frequent vs. infrequent language mixing.
